...
Node.js 22.19 or newer, the repository-pinned Rust 1.89.0 toolchain,
...
public annotated tag remains unchanged as an audit record. The 0.1.2 workflow
also stopped before registry publication or GitHub release creation because
its publisher jobs did not select the supported Python runtime. Its annotated
tag also remains unchanged. The 0.1.3 workflow built every release artifact,
then stopped before obtaining registry credentials, publishing to either
registry, or creating a GitHub release because the checksum verifier rejected
...
changelog. Version 0.1.6 fixes unique-only ingestion and corrects the declared
minimum Rust version. Work from a clean, up-to-date `main`. The
preparation command validates version consistency, the publishable dependency
graph, package contents, release notes, the public source policy, tests, and
documentation. It creates only a local annotated tag:

```sh
./scripts/bump-and-publish 0.1.6 --dry-run --check-history
./scripts/bump-and-publish 0.1.6 --prepare
```
...
```sh
./scripts/bump-and-publish 0.1.6 --push
```
...
The tag workflow publishes version 0.1.6 to crates.io and creates the immutable
...
```sh
./scripts/bump-and-publish 0.1.6 --dispatch-python
```
...
python packaging/bioconda/render_recipe.py \
  --version 0.1.6 \
  --source-archive dist/gravlax-0.1.6-source.tar.gz \
